package SolarAI_MPPT
  model SolarAI "Simple module supplies DC grid with maximum power tracker"
    ...
    PhotoVoltaics.Components.Converters.DCConverter converter annotation(
      Placement(transformation(extent = {{20, -10}, {40, 10}})));
    PhotoVoltaics.Components.Blocks.MPTrackerSample mpTracker(VmpRef = moduleData.VmpRef, ImpRef = moduleData.ImpRef, samplePeriod = 10) annotation(
  ...
  equation
    ...
    connect(battery.pin_n, ground.p) annotation(
      Line(points = {{80, -10}, {80, -15}, {80, -20}}, color = {0, 0, 255}));
    ...
    connect(groundDC.p, converter.dc_n1) annotation(
      Line(points = {{-40, -20}, {-16, -20}, {10, -20}, {10, -6}, {20, -6}}, color = {0, 0, 255}));
    connect(converter.dc_p1, powerSensor.nc) annotation(
      Line(points = {{20, 6}, {10, 6}, {10, 20}, {0, 20}}, color = {0, 0, 255}));
    connect(converter.dc_p2, battery.pin_p) annotation(
      Line(points = {{40, 6}, {50, 6}, {50, 20}, {80, 20}, {80, 10}}, color = {0, 0, 255}));
    connect(converter.dc_n2, ground.p) annotation(
      Line(points = {{40, -6}, {50, -6}, {50, -20}, {80, -20}}, color = {0, 0, 255}));
    annotation(
  ...
  annotation(
    uses(PhotoVoltaics(version = "2.1.0")));
end SolarAI_MPPT;
